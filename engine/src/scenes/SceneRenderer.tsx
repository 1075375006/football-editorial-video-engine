...
export const SceneRenderer: React.FC<{scene: SceneSpec}> = ({scene}) => {
  ...
  switch (scene.pattern) {
    ...
    case 'TEAM-01':
      return <SceneCanvas scene={scene}><div style={{position: 'absolute', left: 80, top: 70, zIndex: 4}}><div style={{fontSize: 22, letterSpacing: 4, color: FEE_COLORS.teamA}}>{info.stats?.team ?? TEAM_A}</div><BigHeadline size={76} start={0}>{info.headline ?? 'FORM WITH A WARNING'}</BigHeadline><div style={{fontSize: 25, maxWidth: 680, marginTop: 24}}>{info.body ?? 'Five unbeaten, but the final third is slowing down.'}</div></div><CutoutSubject name="7" team="A" side="right" sceneDuration={scene.durationInFrames} revealStart={10} /></SceneCanvas>;
    case 'FORM-01':
